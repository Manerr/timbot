...
let syncServerList = (logMembership) => {
  targetChannels = DiscordChannelSync.getChannelList(
    client,
    config.discord_announce_channel,
    logMembership
  );
};
...
axios
  .get("https://twitch.center/customapi/quote/list?token=a912f99b")
  .then((res) => {
    let data = res.data;
    ...
    console.log(
      "[Sellout]",
      `Sellout list initialized from remote, ${selloutList.length} items`
    );
  });
...
class StreamActivity {
  ...
  static updateActivity() {
    ...
    if (streamInfo) {
      ...
      console.log(
        "[StreamActivity]",
        `Update current activity: watching ${streamInfo.user_name}.`
      );
    } else {
    ...
    if (discordChannel) {
      try {
        ...
        if (existingMsgId) {
          ...
          let mentionMode =
            (config.discord_mentions &&
              config.discord_mentions[streamData.user_name.toLowerCase()]) ||
            null;
